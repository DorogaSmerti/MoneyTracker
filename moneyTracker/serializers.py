...
class WalletSerializers(serializers.ModelSerializer):
    user = serializers.StringRelatedField(read_only=True) #Имя пользователя для кошелька
    transaction = TransactionSerializers(many=True, read_only=True) # Транзакция в кошельке
    class Meta:
        model = Wallet
        fields = '__all__'
